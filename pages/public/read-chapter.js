...
class ReadChapter extends React.Component {
  ...
  static async getInitialProps({ req, query }) {
    ...
    // when req exist, showStripeModal is true when the query `?=buy` exist but when
    // req does not exist, showStripeModal is true when window.location.search.includes('buy=1') exist.
    // `location.search` returns the query of a URL, everthing after `?` (?redirectUrl=/book/myfavbook/example?buy=1)
  ...
  componentDidMount() {
    // Detect scroll event and execute a function on every event (this.onScroll)
  ...
  // format_list_bulleted icon toggle
  toggleChapterList = () => {
  ...
  renderMainContent() {
    ...
    return (
      <div style={{ padding }} id="chapter-content">
        ...
        />
        {/* Makes sure that a buy btn does not display on a chapter when chapter.isPurchased or chapter.isFree is true */}
      ...
      <ul>
        {sections.map(s => (
          <li key={s.escapedText} style={{ paddingTop: '10px' }}>
            <a
              style={{
                // if activeSection exist and activeSection.has === s.escapedText(value from the DB chapter.sections.map(s => ...))
                // then highlight (s.text) w/ blue
  ...
  renderSidebar() {
    ...
    // Make sure that renderSideBar() returns null when showTOC is false.
    if (!showTOC) {
      return null;
    }

    const { book } = chapter;
    // chapters obj is part of the book obj
    const { chapters } = book;
        ...
        <ol start="0" style={{ padding: '0 25', fontSize: '14px', fontWeight: '300' }}>
          {chapters.map((ch, i) => (
            <li
              key={ch._id}
              role="presentation"
              // If a list item's index is 0, list style type is none.
              // If a list item's index is anything but 0, list style type is decimal.
  ...
  render() {
    ...
    // const { chapter } = this.state; Remove
    const {
    ...
    return (
      <div style={{ overflowScrolling: 'touch', WebkitOverflowScrolling: 'touch' }}>
        ...
        <div
          ...
          id="main-content" // scroll is detected
        >
